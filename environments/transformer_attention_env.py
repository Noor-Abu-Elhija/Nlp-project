...
from environments.TransformerAttention import TransformerAtenttion
from consts import *


class TransformerAtt_env_ARC(TransformerAtenttion):
...
class TransformerAtt_env(environment.Environment):
    def init_model_arc(self, config):
        self.model = TransformerAtt_env_ARC(n_layers=self.n_layers, tr_layers=self.tr_layers, n_heads=self.n_heads_first,
                                            input_dim=config['input_dim'], hidden_dim=self.hidden_dim,
