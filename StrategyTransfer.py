...
parser.add_argument('--architecture', type=str, default="Final_Model", help='Model architecture')
parser.add_argument('--hidden_dim', type=int, default=32, help='Hidden dimensions')
parser.add_argument('--layers', type=int, default=2, help='Number of layers')
parser.add_argument('--transformer_nheads_first', type=int, default=4, help='Transformer heads')
parser.add_argument('--transformer_nheads_second', type=int, default=1, help='Transformer heads')
parser.add_argument('--transformer_layers', type=int, default=1, help='Number of layers')
...
meta_features_map = {"features": {"EFs": {"FEATURES_PATH": config["SIMULATION_EFs_PATH"], "REVIEW_DIM": 37},
                     ...
                     "architecture": {"LSTM": {"use_user_vector": True},
                                      ...
                                      "Final_Model": {"use_user_vector": True},
                                      "LogReg": {"use_user_vector": False},
                                      "TransformerAttention": {"use_user_vector": True},
                                      "TransformerLSTM": {"use_user_vector": True},
                                      "AttentionLSTM": {"use_user_vector": True},
                                      "Attention": {"use_user_vector": True}
                                      }
                     }
...
elif config["architecture"] == "Final_Model":
    env_model = environments.lstm_transformer_env.LSTM_Transformer_env(env_name, config=config)
elif config["architecture"] == "LogReg":
    env_model = environments.LogRegNN_env.LogReg_env(env_name, config=config)
elif config["architecture"] == "TransformerAttention":
    env_model = environments.transformer_attention_env.TransformerAtt_env(env_name, config=config)
elif config["architecture"] == "TransformerLSTM":
    env_model = environments.TransformerLSTM_env.TransformerLSTM_env(env_name, config=config)
elif config["architecture"] == "AttentionLSTM":
    env_model = environments.AttentionLSTM_env.LSTMAtt_env(env_name, config=config)
elif config["architecture"] == "Attention":
    env_model = environments.Attention_env.Attention_env(env_name, config=config)